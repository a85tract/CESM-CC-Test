...
**Status**: decided 2026-09-24 on the two open points (D7, D8 below); migration steps 1-4 done the same day. D9 decided 2026-09-25: CC-Test is the SciRecast site's results source, and the Cyber move (step 9) is split into 9a-9e and brought forward ahead of step 6.
**Date**: 2026-09-24
...
| 9a | Define the audit output the engine writes and `make_manifest.py` reads: a numbered schema (like the run summary's `schema: 1`), a state beside every count, `NOT_RUN` and `INCOMPLETE` expressible, the scanned commit named. `make_manifest.py` reads it; the `summary.json` from `tools/devsecops-local.sh` (read today through `SCAN_STATE`, `AI_STATE`, `GATE_STATUS`) stays readable until 9e. Agreed with Chien-Wei before any code moves | engine, CC-Test | first, because the manifest's `security` block would otherwise lose its source mid-move |
| 9b | The engine gains an LLM-audit scanner doing what `templates/.github/scripts/ai_audit.py` (418 lines) does: diff → structured verdict → SARIF, registered beside `secret` (gitleaks) and `composition` (syft→grype); the `audit` recipe gains that stage. recast-cesm supplies the prompts (`security/adjudicator.py`) and credentials (`security/provider.py`) it already holds | engine, recast-cesm | the one Cyber algorithm the engine does not yet have |
...
| 10 | `index_evidence.py` also emits `evidence/index.json`; `index_evidence.py --check` covers it; the SciRecast site reads it (that change is in the SciRecast repository, not here) | CC-Test | D9; small, any time after step 4 |
