...
| 9a | The engine's `audit` recipe writes the gate summary CC-Test already reads: `summary.json` in the shape `tools/devsecops-local.sh` writes today (per-scan state and counts, the scanned commit, the gate status; never a finding: a scanner's finding is `PLAUSIBLE` and lands in the local finding store, and it reaches Sec-Track only after adjudication and a human's disclosure decision, none of which CC-Test sees), plus one `schema` field naming the version. No new format: `make_manifest.py` reads this file already (`SCAN_STATE`, `AI_STATE`, `GATE_STATUS`), and the engine's run summary deliberately carries no finding counts, so this is the one artifact the recipe does not yet produce. Agreed with Chien-Wei before any code moves (decided with the maintainer 2026-09-25) | engine | first, because the manifest's `security` block would otherwise lose its source mid-move |
